...
public class EditorStructureObject : MonoBehaviour {
	...
	void Update () {
		if (!set) {
			...
			if (collidingTiles[targetTile].GetComponent<EditorHexBase>().HexTile.GetComponent<EditorTileObject>().tileName == "Hex Blank") {
				collidingTiles[targetTile].GetComponent<EditorHexBase>().structureOnTile = null;
				Destroy(gameObject);
			}
		}
	}
	...
	void OnMouseUp() {
		if (collidingTiles.Count > targetTile) {
			...
			collidingTiles[targetTile].GetComponent<EditorHexBase>().structureOnTile = gameObject;
			set = true;
			targetTile = 0;
		} else {
			Destroy(gameObject);
