...
public class EditorTileObject : MonoBehaviour {

	public string tileName = "Hex Blank";
	...
	void OnMouseDown() {
		if (set) {
			collidingTiles[targetTile].GetComponent<EditorHexBase>().resetTile();
			set = false;
		}
	}
	...
	void OnMouseUp() {
		if (collidingTiles.Count > targetTile) {
			...
			targetTilePos.z = -0.4f;
			transform.position = targetTilePos;
			collidingTiles[targetTile].GetComponent<EditorHexBase>().HexTile = gameObject;
			set = true;
			targetTile = 0;
		} else {
			Destroy(gameObject);
